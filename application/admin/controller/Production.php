...
class Production extends Admin {
    ...
    public function product_build_query() {
        ...
        $lists = model('product_build_order')->model_where()->group('a.id')->paginate(input('get.page_size', 10), $count, ['query' => request()->get()]);
    ...
    public function product_relation_edit($id) {
        ...
        $products = db('product_relation')->alias('a')
                ...
                ->where('a.p_id', $id)
                ->field('a.p_id_bc as id,b.name,b.code,b.image,a.multiple as multiple')
                ->select();
        ...
        if (request()->isPost()) {
            ...
            foreach ($product_ids as $value) {
                ...
                if (!empty($value) && is_numeric($value)) {
                    ...
                    $one = db('product')->where('id', $value)->field('id,name,code,image')->find();
                    $one['multiple'] = 1;
