...
class Res extends Admin {
    ...
    public function _get_list($folder = 'image', $action = '') {
        ...
        foreach($this->ext_arr[$folder] as $val){
          $this->ext_arr[$folder][] = strtoupper($val);
        }        
        ...
        foreach ($images as $image) {
            ...
            if (is_dir($image)) {
                ...
                if ($folder == 'image') {
                    $thumb = img_resize(mb_substr($image, mb_strlen($this->res_dir)), 200, 200);
                } else {
