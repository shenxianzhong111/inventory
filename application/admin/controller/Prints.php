...
class Prints extends Base {
    ...
    public function storage_list() {
        ...
        $this->assign('lists', model('product_storage_order_data')->model_where(true)->select());
    ...
    public function storage_view($id) {
        ...
        $var = model('product_storage_order')->model_where()->where('a.id', $id)->find();

        $sub_list = model('product_storage_order_data')->model_where()->where('a.o_id', $var['id'])->select();
