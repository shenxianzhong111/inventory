...
<form class="form-inline transfer_add"  action="{:url('transfer_add',['id'=>$Think.get.id])}" method="post">
    <table class="table table-hover">
        ...
        <tr>
            ...
            <td>
                <div class="input-prepend input-append">
                    <button type="button" class="btn btn-default" onclick="$('#quantitynumber').val((Number($('#quantitynumber').val()) - 1) < 1 ? 1 : (Number($('#quantitynumber').val()) - 1))"><i class="glyphicon glyphicon-minus"></i></button>
                    <input type="number" id="quantitynumber"  name="number" value="1" placeholder="数量" class="form-control">
                    <button type="button" class="btn btn-default" onclick="$('#quantitynumber').val((Number($('#quantitynumber').val()) + 1) > {$lists.quantity} ? $('#quantitynumber').val() : (Number($('#quantitynumber').val()) + 1))"><i class="glyphicon glyphicon-plus"></i></button>
                </div>
        ...
        <tr>
            ...
            <td>
                <button type="submit" class="btn btn-primary ajax-post" target-form='transfer_add'><i class="glyphicon glyphicon-floppy-disk"></i> 调拨</button>
            </td>
        </tr>
    </table>
</form>
