    ...
    <form class="form-inline" action="{:url('sales_query')}" method="get">
        ...
        <div class="search-box">
            ...
            <button type="button" class="btn btn-default" title="打印出库订单" 
                    id="print"
                    url="<?php echo APP_HOST . url('prints/orders_list', ['session_id' => session_id()]) ?>" 
        ...
        <table class="table table-hover table-striped">
            ...
            <tbody>
                <?php foreach ($lists as $key => $var) {
                    ...
                    <tr{$var.status==='-1'?' class="warning"':($var.status==='-2'?' class="error"':'')} class="product_data_parent" data-id="{$var.id}">
                        <td class="product_dataplus" id="product_dataplus{$var.id}"><i class="glyphicon glyphicon-menu-right"></i></td>
                        ...
                        <td style="text-align:center">
                            <a href="javascript:;" 
                               ...
                               url="<?php echo APP_HOST . url('prints/orders_view', ['id' => $var['id'], 'session_id' => session_id()]) ?>" >打印</a>
                        </td>
                        <td style="text-align:center">
                            <a class="btn btn-primary btn-xs" href="{:url('sales_look',['id'=>$var.id])}" title="查看记录">查看</a></td>
                        <td style="text-align:center">               
                            <a class="btn btn-warning btn-xs ajax-get confirm" href="{:url('sales_undo',['id'=>$var.id])}" title="撤消" >撤消</a>            
                        </td>
                    </tr>
                                    ...
                                    if (!empty($var['child'])) {
                                        foreach ($var['child'] as $key2 => $var2) {
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    {if $var2.status>-2}
                                                    <a class="btn btn-warning btn-xs" data-toggle="modal" data-target="#modal" href="{:url('sales_returns_add',['id'=>$var2.id])}" data-title="产品退货" title="退货">退货</a>
                                                    {/if}
                                                </td>
                                            </tr>
                                            <?php
