...
//            );
    ...
    public function product_sales_query_export($lists) {
        ...
        $PHPSheet->setCellValue('A1', '订单号')
                ...
                ->setCellValue('F1', '数量')
                ->setCellValue('G1', '备注')
                ->setCellValue('H1', '出库类型')
                ->setCellValue('I1', '发货日期')
        ...
        foreach ($lists as $key => $value) {
            ...
            $PHPSheet->setCellValue('H' . $i, $value['type']);
            $PHPSheet->setCellValue('I' . $i, $value['ship_time']);
            $PHPSheet->setCellValue('J' . $i, $value['express_name']);
            $PHPSheet->setCellValueExplicit('K' . $i, $value['express_num'], PHPExcel_Cell_DataType::TYPE_STRING); //防止单元格进行科学计数
            $PHPSheet->setCellValue('L' . $i, $value['nickname']);
            ...
            foreach ($lists_sub as $key2 => $value2) {
                ...
                $PHPSheet->setCellValue('E' . $i, $value2['group_price']);
                $PHPSheet->setCellValue('F' . $i, $value2['quantity']);
                //子行+1
                $i ++;
            }
            //如果存在子行，要合并父行
...
//            );
    ...
    public function finance_export($lists) {
        ...
        foreach ($lists as $key => $value) {
            ...
            $PHPSheet->setCellValue('D' . $i, $value['bank']);
