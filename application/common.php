...
function img_resize($filename, $width = 10, $height = 10, $default = '') {
    ...
    if (is_file($res_path . $new_image)) {
        ...
        if ($width_orig == $width && $height_orig == $height) {
            ...
            $image = new utils\Image\Image($res_path . $old_image);
 ...
 * @return type
 */
function my_md5($value, $salt = APP_SALT) {
 ...
 * @return type
 */
function gen_tree($items, $id = 'id', $pid = 'pid', $son = 'children') {
    // print_r($items);exit;
    $tree = array();
    $tmpMap = array();
